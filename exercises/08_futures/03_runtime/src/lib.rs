...
{
    let response_1 = Arc::new(reply);
    let response_2 = response_1.clone();
    tokio::spawn(async move {
        loop {
            let (mut tcp_stream, _address) = first.accept().await.unwrap();
            tcp_stream
                .write_all(format!("{}", response_1).as_bytes())
                .await
                .unwrap();
        }
    });
    tokio::spawn(async move {
        loop {
            let (mut tcp_stream, _address) = second.accept().await.unwrap();
            tcp_stream
                .write_all(format!("{}", response_2).as_bytes())
                .await
                .unwrap();
        }
    });
